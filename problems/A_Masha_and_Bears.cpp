...
void Remy() {
    ...
    for (int c1 = v1; c1 <= 2 * v1; ++c1) {
        for (int c2 = v2; c2 <= 2 * v2; ++c2) {
            for (int c3 = v3; c3 <= 2 * v3; ++c3) {
                
                bool sie = (c1 > c2 && c2 > c3);
                
               
                bool likes = (v4 <= c3 && 2 * v4 >= c3);
                
            
                
                bool dislike = (v4 <= c2 && 2 * v4 < c2);

                if (sie && likes && dislike) {
                    cout << c1 << el << c2 << el << c3 << el;
                    return;
                }
            }
        }
